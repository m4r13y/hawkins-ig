"use client"

import { motion, AnimatePresence } from "framer-motion"
import { usePathname } from "next/navigation"
import { ReactNode } from "react"

interface PageTransitionProps {
  children: ReactNode
}

const pageVariants = {
  initial: {
    opacity: 0,
    y: 20,
    scale: 0.98
  },
  in: {
    opacity: 1,
    y: 0,
    scale: 1
  },
  out: {
    opacity: 0,
    y: -20,
    scale: 1.02
  }
}

const pageTransition = {
  type: "tween" as const,
  ease: [0.25, 0.46, 0.45, 0.94] as const,
  duration: 0.3 // Reduced duration for faster transitions
}

export default function PageTransition({ children }: PageTransitionProps) {
  const pathname = usePathname()

  // Since layout hardcodes dark mode, always use dark background
  const backgroundClass = "bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950"

  return (
    <div className={`fixed inset-0 ${backgroundClass}`}>
      {/* Persistent background layer to prevent white flash */}
      <div className={`absolute inset-0 ${backgroundClass} z-0`} />
      
      {/* Page content with transitions */}
      <div className="relative z-10 w-full h-full">
        <AnimatePresence mode="wait" initial={false}>
          <motion.div
            key={pathname}
            initial="initial"
            animate="in"
            exit="out"
            variants={pageVariants}
            transition={pageTransition}
            className="w-full h-full page-transition-wrapper overflow-auto"
          >
            {children}
          </motion.div>
        </AnimatePresence>
      </div>
    </div>
  )
}
